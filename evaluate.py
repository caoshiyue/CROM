    ...
    def win_rate(self):
        ...
        for agent in players:
            ...
            for computer in opponents:
                ...
                for result_file in cots:
                    with open(result_file) as fin:
                        results = json.load(fin)["winners"]
                        total = 0
                        win = 0
                        for rnd in results:
                            if int(rnd) > self.exp_rnd:
                                continue
                            if "Alex" in results[rnd]:
                                win += 1
                            total += 1

                        if total > 0:
                            alex_win_rates.append(win / total)

                if alex_win_rates:
                    avg = sum(alex_win_rates) / len(alex_win_rates)
                    std = math.sqrt(sum((x - avg) ** 2 for x in alex_win_rates) / len(alex_win_rates))
                else:
                    avg = 0.0
                    std = 0.0

                win_result[agent][computer] = avg
                win_std[agent][computer] = std

        # 平均胜率和标准差
        average = {}
        stddev = {}
        for agent in win_result:
            values = list(win_result[agent].values())
            stds = list(win_std[agent].values())
            average[agent] = sum(values) / len(values) if values else 0.0
            stddev[agent] = math.sqrt(sum(s ** 2 for s in stds) / len(stds)) if stds else 0.0

        # 输出表格
        print(f"{'':12s}\t" + "\t".join([f"{agent:15s}" for agent in players]))
        for computer in opponents:
            print(f"{computer:12s}", end="\t")
            for agent in win_result:
                avg = win_result[agent][computer]
                std = win_std[agent][computer]
                print(f"{avg:.2f}±{std:.2f}", end="\t")
            print()

        print(f"{'Average':12s}", end="\t")
        for agent in players:
            print(f"{average[agent]:.2f}±{stddev[agent]:.2f}", end="\t")
        print("\n")
...
if __name__=="__main__":
    ...
    parser.add_argument("--result_dir", type=str, default="result_final_part1")
